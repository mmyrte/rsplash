...
splash.point <- function(
  ...
  if (class(tc)[1] %in% c("xts", "zoo", "ts")) {
    # TODO: time() could come from multiple packages (xts, zoo, stats)
    time_index <- time(tc)
  } else if (
    !(class(tc)[1] %in% c("xts", "zoo", "ts")) && !is.null(time_index)
  ) {
    if (verbose) {
      message(
        'Data is not a time-series object, getting time info from "time_index" '
      )
    }
  ...
  time_index_month <- seq(
    time_index[1],
    time_index[length(time_index)],
    "month"
  )
  #### days of the year
  ...
  if (time.freq > 20) {
    time_index <- seq(
      as.Date(paste0(y[1], "-01-01")),
      as.Date(paste0(y[length(y)], "-12-31")),
      "day"
    )
    time_index_month <- seq(
      time_index[1],
    ...
    if (sum(!is.na(sw_in)) < 2) {
      sw_in <- rep(NA, length(time_index))
    } else {
      sw_in <- approx(
        time_index_month,
        sw_in,
  ...
  # The SPLASH symbol in R refers to a c++ object
  ###########################################################################
  my_splash <- methods::new(SPLASH, lat, elev) # nolint
  ###########################################################################
  # get soil hydrophysical characteristics
  ###########################################################################
  soil_info <- soil_hydro(
    sand = soil_data[1],
    clay = soil_data[2],
    OM = soil_data[3],
    fgravel = soil_data[4],
  ...
  initial_AI <- my_splash$spin_up(
    as.integer(365),
    as.integer(y[1]),
    as.numeric(sw_av[1:365]),
    as.numeric(tc_av[1:365]),
    as.numeric(pn_av[1:365]),
    slop,
    asp,
  ...
  if (monthly_out) {
    ...
    ### storages
    result[c("wn", "snow", "sm_lim")] <- lapply(
      result[c("wn", "snow", "sm_lim")],
      FUN = function(x) {
        fastmatch::ctapply(x, INDEX = ind.mont, FUN = mean, na.rm = T)
      }
    )
    result[c("ro", "aet", "pet", "cond", "bflow", "netr")] <- lapply(
      result[c("ro", "aet", "pet", "cond", "bflow", "netr")],
      FUN = function(x) {
        fastmatch::ctapply(x, INDEX = ind.mont, FUN = sum, na.rm = T)
      }
    )
  }

  if (ts_out && !monthly_out) {
    result <- xts::xts(base::as.data.frame(result), time_index)
  } else if (ts_out && monthly_out) {
    result <- xts::xts(base::as.data.frame(result), time_index_month)
  }
...
soil_hydro <- function(sand, clay, OM, fgravel = 0, bd = NA, ...) {
  ...
  if (!is.numeric(sand)) {
    if (is.null(bd)) {
      bd <- (1.5 +
        (dp - 1.5 - 1.10 * (1 - fclay)) * (1 - exp(-0.022 * depth))) /
        (1 + 6.27 * fOM)
    }
  } else {
    bd <- ifelse(
      is.na(bd),
      (1.5 + (dp - 1.5 - 1.10 * (1 - fclay)) * (1 - exp(-0.022 * depth))) /
        (1 + 6.27 * fOM),
      bd
    )
  }
  ...
  bub_init <- -21.6 *
    ...
    27.93 * fclay -
    81.97 * moist_fvol33 +
    71.12 * (fsand * moist_fvol33) +
    8.29 * (fclay * moist_fvol33) +
    14.05 * (fsand * fclay) +
    27.16
  ...
  if (!is.numeric(sand)) {
    ...
    bubbling_p[!is.na(bubbling_p) & bubbling_p > 0] <- coef_A[
      !is.na(bubbling_p) & bubbling_p > 0
    ] *
      -101.97162129779
  }
...
month2day_rain <- function(pn_monthly, ndaypmonth) {
  ...
  if (!is.na(mean_mean) && mean_mean > 0) {
    ...
    if (
      ...
      alph <- (8.898919 + 9.059950 * Y + 0.9775373 * Y^2) /
...
snowfall_prob <- function(tc, lat, elev) {
  ...
  p_snow <- 1 /
    (1 +
      exp(
        -0.4710405934 +
          1.0473543991 * as.numeric(tc) -
          as.numeric(elev) * 0.0004596581 -
          abs(as.numeric(lat)) * 0.0110592101
      ))
  return(p_snow)
}
