...
unSWC.grid <- function(soil_data, uns_depth, wn, d_name, outdir = getwd()) {
  ...
  UnsWater <- function(
    ...
    w_uns_z <- theta_r *
      ...
      (((psi_m + z_uns) *
        (theta_r - theta_s) *
        (bub_press / (psi_m + z_uns))^lambda) /
        (lambda - 1))
    w_uns_0 <- theta_r *
      0 +
      (((psi_m + 0) * (theta_r - theta_s) * (bub_press / (psi_m + 0))^lambda) /
        (lambda - 1))
  ...
  setwd(outdir)
  ### get time info
  y <- as.integer(unique(format(raster::getZ(wn), "%Y")))
  calc_thetai <- function(w, s, r, d) {
    theta_o <- (w / (d * 1000))
    w <- ifelse(
      theta_o >= s,
      s - 0.0001,
      ifelse(theta_o <= r, r + 0.0001, theta_o)
    )
  ...
  theta_i <- raster::overlay(
    ...
    varname = "theta",
    varunit = "m3/m3",
    longname = "volumetric soil moisture",
    xname = "lon",
    yname = "lat",
    zname = "time"
  )
  ...
  psi_m <- bub_press /
    ((((theta_i - theta_r) / (theta_s - theta_r)))^(1 / lambda))
  #########################################################################
  ...
  Se <- raster::overlay(
    ...
    varname = "Se",
    varunit = "fraction",
    longname = "effective saturation or water filled porosity",
    xname = "lon",
    yname = "lat",
    zname = "time"
  )
