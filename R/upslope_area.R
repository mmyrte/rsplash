#' upslope_area
#'
#' Computes n cells draining in/out and the contributing area per pixel in m2 using
#' topmodel for small areas or Taudem for large, corrections acording to latitude done
#' by raster::area
#' @param dem a [RasterLayer][raster::Raster-class] Digital Elevation Model model passed
#' to [topmodel::sinkfill()]
#' @return a RasterStack with the contributing area per pixel, number of cells draining
#' in, and number of cells draining out
#' @keywords contributing area
#' @export
upslope_area <- function(dem, resolution) {
  resolution <- raster::cellStats(resolution, stat = "mean", na.rm = TRUE)
  # 1.0.fill the sinks
  elev <- topmodel::sinkfill(raster::as.matrix(dem), resolution, 1)
  elev <- raster::raster(elev)
  raster::extent(elev) <- raster::extent(dem)
  raster::crs(elev) <- raster::crs(dem)
  # 2. calculate contributing area
  areacatch <- topmodel::topidx(
    raster::as.matrix(elev),
    resolution = resolution
  )$area
  areacatch <- raster::raster(areacatch)
  raster::extent(areacatch) <- raster::extent(elev)
  raster::crs(areacatch) <- raster::crs(elev)
  areacatch <- raster::writeRaster(areacatch, "areacatch.grd", overwrite = TRUE)
  # 3. calculate flow direction and n cells draining in and out
  flowdir <- raster::terrain(elev, opt = "flowdir")
  ncellin <- raster::ncellflow(
    flowdir,
    inout = "in",
    met = "top",
    filename = "ncellin.grd",
    overwrite = TRUE
  )
  ncellout <- raster::ncellflow(
    flowdir,
    inout = "out",
    met = "top",
    filename = "ncellout.grd",
    overwrite = TRUE
  )
...
upslope_areav2 <- function(dem, type, tmpd) {
  ...
  ups_ncell <- raster::raster("dem_a_ac.tif")
  area_p_cell <- raster::area(ups_ncell)
  ...
  ups_ncell <- raster::overlay(
    dem,
    ups_ncell,
    fun = fix_coast,
    filename = "ups_ncell.grd",
    overwrite = TRUE
  )
  ### calculate upslope area
  ups_area <- raster::overlay(
    ups_ncell,
    area_p_cell,
    fun = function(x, y) {
      (x * y * 1e6)
    },
    filename = "areacatch.grd",
    overwrite = TRUE
  )
  # 3. calculate flow direction and n cells draining in and out
  flowdir <- raster::raster("dem_p.tif")

  if (raster::ncell(dem) > 1e7) {
    doParallel::registerDoParallel(raster::getCluster())
    ncellin <- ncellflow_par(
      flowdir,
      inout = "in",
      met = "tau",
      filename = "ncellin.grd",
...
ncellflow_par <- function(flowdir, inout = "in", met = "top", ...) {
  ...
  r <- spatial.tools::focal_hpc(
    x = flowdir,
    fun = compare,
    args = list(inout = inout, met = met),
    window_dims = c(3, 3)
  )
  r
}
