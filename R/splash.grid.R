...
splash.grid <- function(
  ...
  outdir = getwd(),
  tmpdir = dirname(raster::rasterTmpFile()),
  sim.control = list(monthly_out = TRUE, inmem = FALSE)
) {
  ###########################################################################
  ...
  clcheck <- try(raster::getCluster(), silent = TRUE)
  if (class(clcheck)[1] == "try-error") {
    ...
    raster::beginCluster(1, "SOCK")
    message(
      "Only using one core, use first beginCluster(ncores) if you want to run splash in parallel!!"
    )
  }
  ...
  getlatitude <- function(x, filename, ...) {
    ...
    bs <- raster::blockSize(x, minblocks = 200)
    pb <- raster::pbCreate(bs$n)
    pb <- utils::txtProgressBar(min = 1, max = bs$n, style = 1)
    ...
    for (i in 1:bs$n) {
      ...
      xncells <- raster::cellFromRow(x, bs$row[i]:(bs$row[i] + bs$nrows[i] - 1))
      # xmat<-raster::getValues(x,bs$row[i], bs$nrows[i])
      xydata <- raster::xyFromCell(x, xncells)

      # write the chunk of results, bs$row[i] is putting the results in the correct rows
      out <- raster::writeValues(out, xydata[, 2], bs$row[i])
  ...
  if (
    raster::ncell(elev) > 1e7 ||
      (raster::res(elev)[1] > 0.1 && raster::res(elev)[1] < 1)
  ) {
    cat(
      "computing contributing areas and terrain features, it might take a while...",
      "\n"
    )
    start.time <- Sys.time()
    ...
    system(
      "gdaldem aspect -zero_for_flat -compute_edges rawdem.tif aspect_deg.tif"
    )
    terraines <- raster::stack(list(
      slope = "slope_deg.tif",
      aspect = "aspect_deg.tif"
    ))
    # get resolution in m2
    resolution <- raster::area(elev, filename = "area.grd", overwrite = T)
    resolution <- raster::calc(resolution, function(x) {
      sqrt(x) * 1000
    })
    gc()
    ...
    resolution <- sqrt(raster::area(elev)) * 1000
    Au <- upslope_area(elev, resolution)
    # 1.2 get latitudes
    lat <- elev * 0
    lat.data <- raster::rasterToPoints(elev)
    lat[!is.na(lat)] <- lat.data[, 2]
    rm(lat.data)
    terraines <- raster::terrain(
      elev,
      opt = c("slope", "aspect"),
      unit = "degrees"
    )
  ...
  if (is.character(ztime[1])) {
    ...
    # )
    ztime.months <- seq(
      as_BC_date(y[1], month = 1, day = 1),
      as_BC_date(y[length(y)], month = 12, day = 31),
      by = "month"
    )
    ztime.days <- seq(
      as_BC_date(y[1], month = 1, day = 1),
    ...
    ztime.months <- seq(
      as.Date(paste(y[1], 1, sep = "-"), format = "%Y-%j"),
      as.Date(paste(y[length(y)], ny[length(y)], sep = "-"), format = "%Y-%j"),
      by = "month"
    )
    ztime.days <- seq(
      as.Date(paste(y[1], 1, sep = "-"), format = "%Y-%j"),
  ...
  if (!sim.control$inmem) {
    ...
    sm <- raster::brick(
      ...
      varname = "wm",
      varunit = "mm",
      longname = "Soil water content",
      xname = "lon",
      yname = "lat",
      zname = "time"
    )
    sm <- raster::setZ(sm, ztime_out)
    ...
    pet <- raster::setZ(pet, ztime_out)

    aet <- raster::setZ(aet, ztime_out)
    ...
    cond <- raster::brick(
      ...
      varname = "cond",
      varunit = "mm",
      longname = "condensation",
      xname = "lon",
      yname = "lat",
      zname = "time"
    )
    cond <- raster::setZ(cond, ztime_out)
    ...
    bflow <- raster::setZ(bflow, ztime_out)
    ...
    netrad <- raster::brick(
      ...
      varname = "netrad",
      varunit = "MJ",
      longname = "daytime net radiation",
      xname = "lon",
      yname = "lat",
      zname = "time"
    )
    netrad <- raster::setZ(netrad, ztime_out)
    ...
    sm_lim <- raster::brick(
      ...
      varname = "sm_lim",
      varunit = "mm",
      longname = "fraction of available water content",
      xname = "lon",
      yname = "lat",
      zname = "time"
    )
    sm_lim <- raster::setZ(sm_lim, ztime_out)
  } else {
    # actual soil moisture
    sm <- raster::brick(
      nrows = raster::nrow(elev),
      ncols = raster::ncol(elev),
      crs = raster::crs(elev),
      nl = nl
    )
    raster::extent(sm) <- raster::extent(elev)
    # runoff
    ro <- raster::brick(
      nrows = raster::nrow(elev),
      ncols = raster::ncol(elev),
    ...
    raster::extent(ro) <- raster::extent(elev)
    ...
    raster::extent(pet) <- raster::extent(elev)
    ...
    raster::extent(aet) <- raster::extent(elev)
    ...
    raster::extent(swe) <- raster::extent(elev)
    # Condensation
    cond <- raster::brick(
      nrows = raster::nrow(elev),
      ncols = raster::ncol(elev),
    ...
    raster::extent(cond) <- raster::extent(elev)
    # baseflow
    bflow <- raster::brick(
      nrows = raster::nrow(elev),
      ncols = raster::ncol(elev),
    ...
    raster::extent(bflow) <- raster::extent(elev)
    netrad <- bflow
    sm_lim <- bflow
    # move inputs to the memory
    sw_in <- raster::readAll(sw_in)
    tc <- raster::readAll(tc)
    pn <- raster::readAll(pn)
    elev <- raster::readAll(elev)
    lat <- raster::readAll(lat)
    terraines <- raster::readAll(terraines)
    soil <- raster::readAll(soil)
    Au <- raster::readAll(Au)
  }
  ...
  if (nodes > 1) {
    bs <- raster::blockSize(sw_in, minblocks = nodes * 2)
  } else {
    bs <- raster::blockSize(sw_in, minblocks = nodes * 10)
  }
  ...
  pb <- raster::pbCreate(bs$n)
  pb <- utils::txtProgressBar(min = 1, max = max(bs$n, 2), style = 3)
  # cat("computing...","\n")
  ###############################################################################################
  ...
  clFun <- function(i) {
    swrow <- split(
      raster::getValues(sw_in, bs$row[i], bs$nrows[i]),
      1:(ncol(elev) * bs$nrows[i])
    )
    tcrow <- split(
      raster::getValues(tc, bs$row[i], bs$nrows[i]),
      1:(ncol(elev) * bs$nrows[i])
    )
    pnrow <- split(
      raster::getValues(pn, bs$row[i], bs$nrows[i]),
      1:(ncol(elev) * bs$nrows[i])
    )
    ...
    soilrow <- split(
      raster::getValues(soil, bs$row[i], bs$nrows[i]),
      1:(ncol(elev) * bs$nrows[i])
    )
    Aurow <- split(
      raster::getValues(Au, bs$row[i], bs$nrows[i]),
      1:(ncol(elev) * bs$nrows[i])
    )
    ...
    splash_block <- mapply(
      ...
      MoreArgs = list(
        ts_out = FALSE,
        monthly_out = sim.control$monthly_out,
        time_index = ztime,
        verbose = FALSE
      ),
      SIMPLIFY = TRUE
  ...
  if (!sim.control$inmem) {
    ...
    matsm <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matro <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matpet <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    mataet <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matswe <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matcond <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matbflow <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matnetrad <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
    matsm_lim <- matrix(ncol = raster::nlayers(sm), nrow = raster::ncell(sm))
  ...
  for (i in 1:bs$n) {
    ...
    if (!sim.control$inmem) {
      sm <- raster::writeValues(
        sm,
        do.call(rbind, d$value$value[1, ]),
        bs$row[b]
      )
      ro <- raster::writeValues(
        ro,
        do.call(rbind, d$value$value[2, ]),
        bs$row[b]
      )
      pet <- raster::writeValues(
        pet,
        do.call(rbind, d$value$value[3, ]),
        bs$row[b]
      )
      aet <- raster::writeValues(
        aet,
        do.call(rbind, d$value$value[4, ]),
        bs$row[b]
      )
      swe <- raster::writeValues(
        swe,
        do.call(rbind, d$value$value[5, ]),
        bs$row[b]
      )
      cond <- raster::writeValues(
        cond,
        do.call(rbind, d$value$value[6, ]),
        bs$row[b]
      )
      bflow <- raster::writeValues(
        bflow,
        do.call(rbind, d$value$value[7, ]),
        bs$row[b]
      )
      netrad <- raster::writeValues(
        netrad,
        do.call(rbind, d$value$value[8, ]),
        bs$row[b]
      )
      sm_lim <- raster::writeValues(
        sm_lim,
        do.call(rbind, d$value$value[9, ]),
        bs$row[b]
      )
    } else {
  ...
  if (!sim.control$inmem) {
    sm <- raster::writeStop(sm)
    ro <- raster::writeStop(ro)
    pet <- raster::writeStop(pet)
    aet <- raster::writeStop(aet)
    swe <- raster::writeStop(swe)
    cond <- raster::writeStop(cond)
    bflow <- raster::writeStop(bflow)
    netrad <- raster::writeStop(netrad)
    sm_lim <- raster::writeStop(sm_lim)
  } else {
    # soil water content
    sm <- raster::setValues(sm, matsm)
    sm <- raster::setZ(sm, ztime_out)
    # runoff
    ro <- raster::setValues(ro, matro)
    ro <- raster::setZ(ro, ztime_out)
    # Potential evapotranspiration
    pet <- raster::setValues(pet, matpet)
    pet <- raster::setZ(pet, ztime_out)
    # Actual evapotranspiration
    aet <- raster::setValues(aet, mataet)
    aet <- raster::setZ(aet, ztime_out)
    # Snow water equivalent
    swe <- raster::setValues(swe, matswe)
    swe <- raster::setZ(swe, ztime_out)
    # Condensation
    cond <- raster::setValues(cond, matcond)
    cond <- raster::setZ(cond, ztime_out)
    # baseflow
    bflow <- raster::setValues(bflow, matbflow)
    bflow <- raster::setZ(bflow, ztime_out)
    # net radiation
    netrad <- raster::setValues(netrad, matnetrad)
    netrad <- raster::setZ(netrad, ztime_out)
    ## fraction FC
    sm_lim <- raster::setValues(sm_lim, matsm_lim)
    sm_lim <- raster::setZ(sm_lim, ztime_out)
  }
